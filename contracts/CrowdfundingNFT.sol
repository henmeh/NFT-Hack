...
contract CrowdfundingNFT is ERC721 {    
    ...
    function mint(uint8 _isSupertoken, uint256 _streamAmount, uint256 _streamRate, address _streamToken, address _erc20TokenAddress, address _projectOwner) public returns(bool) {      
        ...
        if(_isSupertoken == 2) {
            ...
            require(IERC20(_streamToken).balanceOf(msg.sender) > (_streamAmount + _securityAmount), "Not enough funds in your wallet to pay for stream and security");
        ...
        allNFTs[tokenId] = nftData(_startingBlockTime, 0, _projectOwner, _streamAmount, _streamRate, _streamToken, _securityAmount, msg.sender, false, _streamAmount);
    ...
    function stopFlow(uint256 _tokenId) public returns(bool) {
        ...
        require(allNFTs[_tokenId].minter == msg.sender || allNFTs[_tokenId].minter == operator, "You are not allowed to stop the stream");
    ...
    function withdraw(uint256 _tokenId) public returns(bool) {
        ...
        require(allNFTs[_tokenId].minter == msg.sender || allNFTs[_tokenId].minter == operator, "You are not allowed to withdraw");
